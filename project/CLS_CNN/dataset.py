...
class DatasetPredCropped(Dataset):
    def __init__(self, transform=None, name_patients=None, y_patients=None, run="run4", pil = False):                
        self.root = "/fhome/mapsiv/QuironHelico/CroppedPatches"
...
class DatasetPredCropped3(Dataset):
    def __init__(self, transform=None, name_patients=None, y_patients=None, run="run4", pil=False):                
        self.root = "/fhome/mapsiv/QuironHelico/CroppedPatches"
        self.root_pred = f"/fhome/gia07/project/runs/{run}/CroppedPatches"
        self.name_pacients = name_patients
...
def create_dataloader_predicted(path_dict, transform, batch_size, annotated=True, shuffle=True, run="run4", pil=False):
    ...
    if annotated:
        ...
        dataset = DatasetPredCropped(transform, pacient_ids, labels, run=run, pil = pil)
    dataloader = torch.utils.data.DataLoader(dataset, batch_size=batch_size, shuffle=shuffle)
    return dataloader
    

def create_dataloader_predicted_cnn(path_dict, transform, batch_size, annotated=True, shuffle=True, run="run4"):
    with open(path_dict, 'rb') as file:
        pacient_ids, labels = pickle.load(file)
